...
<?php if (!isset($_GET['id'])): ?>
    ...
    <section class="all-villas">
        <div class="all-villas__filters-wrapper">
            <button type="button" class="all-villas__filters-toggle">
                <p>Filters ☰</p>
            </button>
            <form method="GET" action="villa.php" class="all-villas__form-filters">
            <h3 class="all-villas__filter-title">Prijs</h3>
            <div class="all-villas__price-setting">
                <input type="number" name="min_price" placeholder="Min. prijs" value="<?= htmlspecialchars($_GET['min_price'] ?? '') ?>" class="all-villas__price-input">
                <input type="number" name="max_price" placeholder="Max. prijs" value="<?= htmlspecialchars($_GET['max_price'] ?? '') ?>" class="all-villas__price-input">
            </div>
            <!-- Dynamic Ligging Options -->
            <fieldset class="all-villas__fieldset-liggings">
                <h3 class="all-villas__filter-title">Ligging</h3>
            ...
            <fieldset class="all-villas__fieldset-eigenschappen">
                <h3 class="all-villas__filter-title">Eigenschappen</h3>
                ...
                <?php foreach ($villas as $item): ?>
                    <div class="all-villas__item">
                        <a href="villa.php?id=<?= htmlspecialchars($item->id) ?>" class="all-villas__item-button">
                            ...
                            <?php else: ?>
                                <img src="../assets/img/villa/no_img_avaible.jpg" alt="Geen afbeelding" style="width:100%;height:auto;margin-bottom:10px;" class="all-villas__item-image">
                            <?php endif; ?>
